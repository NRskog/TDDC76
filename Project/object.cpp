...
{
    [[maybe_unused]] auto unused_context = context;
    //För att ta bort objekt från context vektorn. 
    if(self_hp <= 0)
    {
        return true;
    }
...
{   //skjuter med en viss sannolikhet som ökar baserat på tiden som gått.
    ...
    if (shoot_prob >= int(context.self_context_json_data.self_jsonData["Objects"]["Octopus"]["probability"].at(1 + context.self_context_difficulty)) - context.self_context_time.getElapsedTime().asSeconds())
    {
...
void Octopus::shoot_projectile(Context & context) const
{
    context.self_context_objects_temp.push_back(new Projectile{
            ...
            self_object_sprite.getPosition().x,
            self_object_sprite.getPosition().y,
            context.self_context_json_data.self_jsonData["Objects"]["Projectile_enemy"]["speed"].at(0 + context.self_context_difficulty),
